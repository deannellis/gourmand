  ...
  $.fn.dynamiclist = function (options) {
    ...
    var handleAdd = function (list, event, settings) {
      ...
      if (length < settings.maxSize) {
        ...
        var item = list
          .find("." + settings.itemClass + ":first")
          .clone(settings.withEvents);
        ...
        if (settings.addCallbackFn != null) settings.addCallbackFn(item);
      }
    ...
    var handleRemove = function (list, alink, event, settings) {
      ...
      if (length == settings.minSize) clearItem(item);
      else item.remove();

      normalizeList(list, settings);

      if (settings.removeCallbackFn != null) settings.removeCallbackFn(item);
    ...
    var init = function (list) {
      ...
      list
        .find("." + settings.itemClass + ":first " + "." + settings.removeClass)
        .hide();
